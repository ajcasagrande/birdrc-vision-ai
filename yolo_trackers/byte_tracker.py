...
class BYTETracker:
    ...
    def __init__(self, args, frame_rate=30):
        ...
        self.reset_id()
        # self.args.max_reactivation_distance = 400
        # self.args.match_thresh_second =  0.8 # 0.5
        # self.args.match_thresh_third = 0.9 # 0.7
    ...
    def update(self, results, img=None):
        ...
        matches, u_unconfirmed, u_detection = matching.linear_assignment(dists, thresh=self.args.match_thresh_third)
        for itracked, idet in matches:
        ...
        for inew in u_detection:
            ...
            # Check for nearby lost tracks
            min_distance = float('inf')
            closest_lost_track = None
            for lost_track in self.lost_stracks:
                distance = np.linalg.norm(lost_track.xywh[:2] - det.xywh[:2])  # Euclidean distance
                print(f'*** {distance=}')
                if distance < self.args.max_reactivation_distance and distance < min_distance:
                    min_distance = distance
                    closest_lost_track = lost_track

            if closest_lost_track:
                # Reactivate the closest lost track
                closest_lost_track.re_activate(det, self.frame_id)
                activated_stracks.append(closest_lost_track)
                print(f' *** re-tracking track_id {closest_lost_track.track_id}')
            else:
                if det.score < self.args.new_track_thresh:
                    print(f'*** ignoring new detection with score {det.score}')
                    continue
